...
public:
                  ...
                  const MemoryBlock* postData) override
    {
        ...
        if (trimmed.startsWithIgnoreCase ("file:"))
        {
            ...
            if (NSURL* nsUrl = [NSURL fileURLWithPath: juceStringToNS (file.getFullPathName())])
            {
                // Resolve symlinks so paths stay consistent for sandbox / temp dirs.
                if (NSURL* resolved = [nsUrl URLByResolvingSymlinksInPath])
                    nsUrl = resolved;

                const auto ext = file.getFileExtension().toLowerCase();

                // Embedded WKWebView often shows a blank white page with loadFileURL for local HTML.
                // loadHTMLString avoids file-read-access edge cases while keeping a sensible origin via baseURL.
                if (ext == ".html" || ext == ".htm")
                {
                    NSError* err = nil;
                    NSString* html = [NSString stringWithContentsOfURL: nsUrl
                                                              encoding: NSUTF8StringEncoding
                                                                 error: &err];

                    if (html != nil && [html length] > 0)
                    {
                        NSURL* baseURL = [nsUrl URLByDeletingLastPathComponent];

                        if (NSURL* rb = [baseURL URLByResolvingSymlinksInPath])
                            baseURL = rb;

                        [webView.get() loadHTMLString: html baseURL: baseURL];
                        return;
                    }
                }

                NSURL* readAccessURL = [nsUrl URLByDeletingLastPathComponent];

                if (NSURL* resolvedRead = [readAccessURL URLByResolvingSymlinksInPath])
                    readAccessURL = resolvedRead;

                if (readAccessURL == nil)
                    readAccessURL = nsUrl;

                [webView.get() loadFileURL: appendParametersToFileURL (url, nsUrl) allowingReadAccessToURL: readAccessURL];
            }
        }
